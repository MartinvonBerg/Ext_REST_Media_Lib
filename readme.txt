...
== Description ==
This plugin extends the REST-API of Wordpress to directly access the Media-Library for Images. It is intended to be used together with a Lightroom Plugin. The new REST-API endpoints (functions) enable to add additional metadata to images, update existing metadata or update images completely without changing the Wordpress-ID. Images may be added to the standard directory hierarchy of wordpress or -even better- an additional folder hierarchy which allows better organization and searching for images.
...
1. Use WP-Admin User and Password + Basic-Auth
...
This works only with wordpress 5.6+ and may be used together with Basic-Auth. The setting is only provided if your website runs with https. So, use it only together with https (see above). I prefer this method and recommend to update to WP 5.6. 
Process:
 ...
 - Use the username of the admin and this Password in the https-header to access to wordpress 
...
How to get this: Open you browser and type https://www.your-domain.whatever/wp-json/wp/v2/media. Use Firefox to get a formatted output of the response.
...
The https GET-Request 'https://example.com/wp-json/wp/v2/media/?_fields=id,gallery'
provides for example this response: 
...
Note on image resizing: Wordpress sets the standard resize quality to 82%. A setting of 100% was tested but with that the image-files were rather big. The setting may be changed by the code only. Up to now there is now administration panel for the settings of this plugin.
...
The new JPG-file has to be provided in the body as binary string. Checks in mime-type and size are done to prevent the user from uploading wrong images.
...
There is no need to upgrade right now. 
